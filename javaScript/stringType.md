...
- 유니코드 문자들에는 모두 Unicode Code Point라고 불리는 숫자(=번호)가 부여되어 있는데, 이것을 문자의 키라고 생각할 수 있다.
    ...
    가 : U+AC00
    ```
    : 이는 한글 '가' 문자에 부여된 유니코드 코드 포인트 이다.
    
    : 번호의 접두어에 `U+`가 붙어있으면 해당 문자가 유니코드 문자라는 뜻이며, 뒤에 오는 4자리 혹은 이상의 번호는 16진수로 되어있다.

    : [유니코드 표](http://graphemica.com/)

#### utf-8 과 utf-16

- 위에서 본 이러한 **유니코드**를 0과 1로 이루어진 컴퓨터의 언어처럼 2진수로 변환을 하게 되는데, 이 변환 과정 자체를 '**인코딩(부호화)**'라 부르며, `utf-8`과 `utf-16`은 인코딩 방식 중 하나인 것이다.
...
- 리터럴을 위해 사용한 따옴표와 다른 종류의 따옴표들은 문자열 내에서 자유롭게 쓸 수 있지만 같은 종류의 따옴표는 내부적으로 포함할 수 없다. 
    ...
    ```
    이는 cat의 앞 따옴표와 dog의 앞 따옴표를 한 쌍으로 간주하여 "cat  " 리터럴로 인식하게 되는 형식으로 진행되기 때문에 문법 에러를 발생시킨다.
...
- 따옴표는 escape sequence로 포함해도 되기는 하지만 다른 종류의 따옴표라면 문자열 안에 자유롭게 포함할 수 있기 때문에 굳이 escape sequence로 사용하지 않아도 될 것 같다. 
    ...
    '\'cat\'' or "'cat'"
    ```
- `\n` 과 `\r` 은 둘 다 개행문자이며 운영 체제마다 엔터를 입력하는 문자가 달라 두개가 생긴 것이지 기능의 차이점은 없다. 현재 맥과 리눅스는 `LF`, 윈도우는 `CR+LF`가 개행문자로 사용되고 있다.
...
단, 단순 문자열 연결은 + 연산자를 사용해도 되기 때문에 선택할 수 있다.
```
...
예 ) 'hello'.indexOf('h') !== -1
    true면 포함, false면 값이 -1이라는 뜻이니 미포함
*/
```
```js
...
혹은, 문자 사이 간격에다가 0부터의 인덱스를 부여하고,
x번틈과 y번틈 사이 문자열을 자르겠다라고 생각해도 된다.
...
# 2. Today I Found Out
```
아직 간단한 코드 작성할 때도 버벅거리지만.. 그래도 재밌으니 다행인가
유니코드 인코딩 방식 등 컴퓨터 사이언스 관련한 지식이 별로 없으니 
좀 답답하고 뭔가 물 위에서만(?) 공부하는 것 같은 느낌이 들어서 
